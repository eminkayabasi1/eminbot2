...
public class AmzNewSearcherThread implements Runnable {
    ...
    public void run() {
        ...
        for (int j = 1; j > 0; j++) {
            try {
                ...
                urunHashMap = amzUrunToplaFonksiyon.menudenUrunBul(amzKategori, 1, null, urunHashMap, j < 5, yasakliUrunHashMap, mesajHashMap);
                //System.out.println("AMZ New Searcher Tur Bitti " + urunHashMap.size() + " - " + amzKategori.getKategoriAdi() + " - j " + j);
                /**Random rand = new Random();
                int sleepRand = rand.nextInt(2);
                TimeUnit.MINUTES.sleep(sleepRand + 1);*/
            } catch (Exception e) {
                //System.out.println("AMZ New Searcher Thread hata aldım: " + amzKategori.getKategoriAdi() + " - " + sdf.format(new Date()));
